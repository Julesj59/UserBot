...
PRO_STRINGS = [
     "`This boi is pro as phack.`",
     "`All peru, me super peru. Huihui.`",
]
ABUSEHARD_STRING = [
    ...
    if not hacked.text[0].isalpha() and hacked.text[0] not in ("/", "#", "@", "!"):
        if await hacked.get_reply_message():
            await hacked.edit(
                ...
                "`Buy a panda for` @sunidhijain `To Remove This Hack.`\n"
            )
...
async def killing (killed):
    ...
    if not killed.text[0].isalpha() and killed.text[0] not in ("/", "#", "@", "!"):
        if await killed.get_reply_message():
            await killed.edit(
                "`Targeted User was murdered and evidences cleared. 😈`\n"
            )
